...
/* }}} */
/*lux: parse class method parameters */
...
ZEND_API int zend_startup_module_ex(zend_module_entry *module TSRMLS_DC) /* {{{ */
{
	...
	if (module->module_startup_func) {
		...
		if (module->module_startup_func(module->type, module->module_number TSRMLS_CC)==FAILURE) {//lux: 调用模块的m函数
...
ZEND_API int zend_startup_modules(TSRMLS_D) /* {{{ */
{
	...
	zend_hash_apply(&module_registry, (apply_func_t)zend_startup_module_ex TSRMLS_CC);//lux：模块初始化
	return SUCCESS;
}
/* }}} */
...
ZEND_API void zend_activate_modules(TSRMLS_D) /* {{{ */
{
	...
	while (*p) {
		...
		if (module->request_startup_func(module->type, module->module_number TSRMLS_CC)==FAILURE) {//lux：调用模块的r函数
...
{//lux: 声明class属性
	...
	if (access_type & ZEND_ACC_STATIC) {//lux:静态变量
	...
	if (ce->type & ZEND_INTERNAL_CLASS) {//lux：内部class
