...
{//lux: zend开始啦
	...
	start_memory_manager(TSRMLS_C);//lux: zend内存管理初始化
...
/* build with dtrace support */
	...
	zend_init_opcodes_handlers();//lux: 初始化opcode handlers

	/* set up version */
	zend_version_info = strdup(ZEND_CORE_VERSION_INFO);
	zend_version_info_length = sizeof(ZEND_CORE_VERSION_INFO) - 1;

	GLOBAL_FUNCTION_TABLE = (HashTable *) malloc(sizeof(HashTable));//lux: 初始化全局函数表, 用来装载内部函数如print_r
	GLOBAL_CLASS_TABLE = (HashTable *) malloc(sizeof(HashTable));//lux: 初始化全局类表
	GLOBAL_AUTO_GLOBALS_TABLE = (HashTable *) malloc(sizeof(HashTable));//lux:you know
	GLOBAL_CONSTANTS_TABLE = (HashTable *) malloc(sizeof(HashTable));//lux: 全局常量表
	...
	*executor_globals->zend_constants = *GLOBAL_CONSTANTS_TABLE;
	...
	zend_startup_builtin_functions(TSRMLS_C);//lux: 注册内部函数，method_exists... 见zend_builtin_functions.c:243
	zend_register_standard_constants(TSRMLS_C);//lux: 注册标准常量,e_error... 见zend_constants.c:96
...
ZEND_API void zend_deactivate(TSRMLS_D) /* {{{ */
{
	...
	if (GC_G(gc_enabled) && !CG(unclean_shutdown)) {
		gc_collect_cycles(TSRMLS_C);//lux:主动调用垃圾回收
	}
#endif
...
{//lux:zend execute 开始
	...
	for (i = 0; i < file_count; i++) {//lux:开始逐个解析文件
		...
		EG(active_op_array) = zend_compile_file(file_handle, type TSRMLS_CC);//lux:生成opcode, 文件 =>lex&parse =>opcode
		...
		if (EG(active_op_array)) {
			...
			zend_execute(EG(active_op_array) TSRMLS_CC);//lux:执行opcode
	...
	va_end(files);//lux:zend execute 执行结束
	EG(active_op_array) = orig_op_array;
	EG(return_value_ptr_ptr) = orig_retval_ptr_ptr;
    CG(interactive) = orig_interactive;
