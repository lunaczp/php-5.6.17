...
#define _CONST_CODE  0 //lux:对应zend_compile.h IS_CONST
#define _TMP_CODE    1
#define _VAR_CODE    2
...
{
	zval ***ptr = EX_CV_NUM(execute_data, var);//lex:todo
	...
	if (Z_TYPE_P(variable_ptr) == IS_OBJECT &&
 	...
 	if (EXPECTED(!PZVAL_IS_REF(variable_ptr))) {
		if (Z_REFCOUNT_P(variable_ptr)==1) {
			if (UNEXPECTED(variable_ptr == value)) {//lux: $a=10;$a=$a;
				return variable_ptr;
			} else if (EXPECTED(!PZVAL_IS_REF(value))) {//lux: $a=10;$b=$a;
				Z_ADDREF_P(value);
				*variable_ptr_ptr = value;
				...
				efree(variable_ptr);
				return value;
			} else {//lux: $a=10;$c=&$a;$b=$a;
				goto copy_value;
			}
		} else { /* we need to split */
			Z_DELREF_P(variable_ptr);//lux: 因为要赋别的值了，当前指向的zval的count自然要减一
			GC_ZVAL_CHECK_POSSIBLE_ROOT(variable_ptr);
			if (PZVAL_IS_REF(value)) {//lux: $a=10;$b=$a; $x=10;$y=&$x; $b=$x; 给b分配新的内存，把x的zval拷贝一份给b
				ALLOC_ZVAL(variable_ptr);
				*variable_ptr_ptr = variable_ptr;
				INIT_PZVAL_COPY(variable_ptr, value);
				zval_copy_ctor(variable_ptr);
				return variable_ptr;
			} else {//lux: $a=10;$b=$a; $x=10; $b=$x; 直接把b指向x的zval
				*variable_ptr_ptr = value;
				Z_ADDREF_P(value);
				return value;
			}
...
copy_value:
			if (EXPECTED(Z_TYPE_P(variable_ptr) <= IS_BOOL)) {//lux: 对于简单变量，无需垃圾回收，直接拷贝即可；否则，进行垃圾回收
				/* nothing to destroy */
				ZVAL_COPY_VALUE(variable_ptr, value);
				zendi_zval_copy_ctor(*variable_ptr);
			} else {
