...
{//lux:文件解析入口
	...
	zend_op_array *op_array = (zend_op_array *) emalloc(sizeof(zend_op_array));//lux:分配内存，提供一个opcode array,用来存生成的opcode array
	...
	if (open_file_for_scanning(file_handle TSRMLS_CC)==FAILURE) {
		...
		init_op_array(op_array, ZEND_USER_FUNCTION, INITIAL_OP_ARRAY_SIZE TSRMLS_CC);//lux:初始化opcode 数组
		CG(in_compilation) = 1;
		CG(active_op_array) = op_array;//lux:把初始化好的opcode array复制到全局
		zend_stack_push(&CG(context_stack), (void *) &CG(context), sizeof(CG(context)));
		zend_init_compiler_context(TSRMLS_C);
		compiler_result = zendparse(TSRMLS_C);//lux:parse 开始
...
#line 4763 "Zend/zend_language_scanner.c"
yy410:
...
#line 6904 "Zend/zend_language_scanner.c"
yy761:
