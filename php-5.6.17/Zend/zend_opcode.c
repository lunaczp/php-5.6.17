...
{//lux: 如题，初始化一个op array
	...
	if (CG(interactive)) {
		 ...
		 * will become invalid
		 */
		initial_ops_size = INITIAL_INTERACTIVE_OP_ARRAY_SIZE;//lux: 所以直接来个大值（如果仍然溢出呢 todo）
	}
	...
	*op_array->refcount = 1;
	...
	op_array_alloc_ops(op_array, initial_ops_size);//lux:分配opcodes 数组内存,(64)
...
{//lux: op array的信息填充
	...
	while (opline < end) {//lux:对opcode 数组，填充对应的对handler
