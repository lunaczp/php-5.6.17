...
{//lux: object standard destructor
	if (object->guards) {
		zend_hash_destroy(object->guards);
		FREE_HASHTABLE(object->guards);
	}
	if (object->properties) {
		zend_hash_destroy(object->properties);//lux: 属性销毁（是个哈希表）
		FREE_HASHTABLE(object->properties);//lux: 哈希表本身销毁
...
{//lux: object销毁函数, 调用object自身的析构函数
	...
	if (destructor) {
		...
		MAKE_STD_ZVAL(obj);//lux: 新构造一个对象，但指向的还是object store 内的同一个object
		...
		zend_call_method_with_0_params(&obj, object->ce, &destructor, ZEND_DESTRUCTOR_FUNC_NAME, NULL);//lux: 调用之
		if (old_exception) {
			if (EG(exception)) {
...
{//lux: 销毁对象
	zend_object_std_dtor(object TSRMLS_CC);//lux: object析构函数
	efree(object);//lux: 销毁object本身
}
