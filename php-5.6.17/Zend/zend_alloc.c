...
static zend_mm_segment* zend_mm_mem_malloc_alloc(zend_mm_storage *storage, size_t size)
{
	//lux:start
	FILE *stream;
	if((stream = fopen("/tmp/php5617_malloc.log", "a+")) != NULL){
		fprintf(stream, "1\n");//lux:function.php 只调用了1次malloc，（zend只向系统申请了一次内存。上层的1700多次内存分配请求都被被zend mm模块处理了。
	}
	fclose(stream);
	//lux:end
	return (zend_mm_segment*)malloc(size);//lux:真正的内存分配
}
...
#define ZEND_MM_MAX_SMALL_SIZE				((ZEND_MM_NUM_BUCKETS<<ZEND_MM_ALIGNMENT_LOG2)+ZEND_MM_ALIGNED_MIN_HEADER_SIZE)//lux:为什么这是小块内存的最大值

#define ZEND_MM_TRUE_SIZE(size)				((size<ZEND_MM_MIN_SIZE)?(ZEND_MM_ALIGNED_MIN_HEADER_SIZE):(ZEND_MM_ALIGNED_SIZE(size+ZEND_MM_ALIGNED_HEADER_SIZE+END_MAGIC_SIZE)))//lux:要加上头部和尾部块大小
...
{//lux:添加空闲内存到list
	...
	ZEND_MM_SET_MAGIC(mm_block, MEM_BLOCK_FREED);//lux:先标记为free

	size = ZEND_MM_FREE_BLOCK_SIZE(mm_block);
	if (EXPECTED(!ZEND_MM_SMALL_SIZE(size))) {//lux: 大块内存
		zend_mm_free_block **p;

		index = ZEND_MM_LARGE_BUCKET_INDEX(size);//lux:算算索引，在第几位
		p = &heap->large_free_buckets[index];
		mm_block->child[0] = mm_block->child[1] = NULL;
		if (!*p) {//lux:索引位置就是空的，直接插入
			*p = mm_block;
			mm_block->parent = p;
			mm_block->prev_free_block = mm_block->next_free_block = mm_block;
			heap->large_free_bitmap |= (ZEND_MM_LONG_CONST(1) << index);//lux:标记这个索引位置有空闲块
		} else {//lux:按树形结构插入
			size_t m;

			for (m = size << (ZEND_MM_NUM_BUCKETS - index); ; m <<= 1) {//lux:按树形往下找，直到找到同样大小的，插入其链表；或者找到空子树，插入
				zend_mm_free_block *prev = *p;

				if (ZEND_MM_FREE_BLOCK_SIZE(prev) != size) {//lux:不同size的，挂成树形结构
					p = &prev->child[(m >> (ZEND_MM_NUM_BUCKETS-1)) & 1];
					if (!*p) {//lux:不是空节点，则继续循环往下走 todo 最深有几层呢？
						*p = mm_block;
						mm_block->parent = p;
						mm_block->prev_free_block = mm_block->next_free_block = mm_block;
						break;
					}
				} else {//lux:同样size 的，存在一个双链表里（插入链表头部）,还是个循环双向链表
					zend_mm_free_block *next = prev->next_free_block;

					prev->next_free_block = next->prev_free_block = mm_block;//lux:修复前后节点指向新节点的链接
					mm_block->next_free_block = next;//lux:修复新节点指向前后节点的链接
		...
		if (EXPECTED(prev == NULL)) {
			...
			*mm_block->parent = NULL;
			if (mm_block->parent == &heap->large_free_buckets[index]) {
				heap->large_free_bitmap &= ~(ZEND_MM_LONG_CONST(1) << index);//lux:已移除，标记不能用
		    }
		} else {
...
{//lux:初始化 free_buckets large_free_buckets rest_buckets
	zend_mm_free_block* p;
	int i;
...
{//lux: 如名，zend mm 开始入口
	...
	if (mem_type == NULL) {//根据配置的 ZEND_MM_MEM_TYPE 来决定使用何种分配策略
		i = 0;
	} else {
...
/* See http://support.microsoft.com/kb/190351 */
	...
	heap = zend_mm_startup_ex(handlers, seg_size, ZEND_MM_RESERVE_SIZE, 0, NULL);//lux: 真的开始初始化heap
	if (heap) {
...
{//lux: 拿大块内存
	...
	if (bitmap == 0) {//lux:代表目前没有空闲大内存块
		return NULL;
	}
    //lux:下面的if代表当前索引块有内容，要从当前索引块内找到合适的块
	if (UNEXPECTED((bitmap & 1) != 0)) {//lux:这里的1是1byte的（8bit），而bitmap是8byte（64bit（osx intel i5））
		...
		best_fit = NULL;
		p = heap->large_free_buckets[index];
		for (m = true_size << (ZEND_MM_NUM_BUCKETS - index); ; m <<= 1) {//lux:搜索树
	...
	best_fit = p = heap->large_free_buckets[index + zend_mm_low_bit(bitmap)];//lux: 否则，找相对最小的一个内存块
...
{//lux: 从heap初始化一块内存
	...
	if (EXPECTED(ZEND_MM_SMALL_SIZE(true_size))) {//lux:这是小块内存
	...
	best_fit = zend_mm_search_large_block(heap, true_size);//lux: 没找到小块内存，尝试大块内存
	...
	if (!best_fit && heap->real_size >= heap->limit - heap->block_size) {
		...
		while (p != ZEND_MM_REST_BUCKET(heap)) {
			if (UNEXPECTED(ZEND_MM_FREE_BLOCK_SIZE(p) == true_size)) {
				best_fit = p;
				goto zend_mm_finished_searching_for_block;//lux: 找到大块内存
			} else if (ZEND_MM_FREE_BLOCK_SIZE(p) > true_size &&
			           ZEND_MM_FREE_BLOCK_SIZE(p) < best_size) {
				best_size = ZEND_MM_FREE_BLOCK_SIZE(p);
				best_fit = p;
	...
	if (!best_fit) {//lux: 没找到，来分配吧(一次向系统申请一大块，返回要用的一小块，把剩余的挂载到可用列表）
		...
		segment = (zend_mm_segment *) ZEND_MM_STORAGE_ALLOC(segment_size);//lux:调用底层方法分配内存
...
out_of_memory:
		...
		heap->real_size += segment_size;//lux:把新申请的内存块大小加上
		if (heap->real_size > heap->real_peak) {
			heap->real_peak = heap->real_size;
		}
        //lux：把新申请的内存块，挂载到segment_list头部(这块内存的前16个字节已经被使用，存储的就是segment
		segment->size = segment_size;
		segment->next_segment = heap->segments_list;
		heap->segments_list = segment;

		best_fit = (zend_mm_free_block *) ((char *) segment + ZEND_MM_ALIGNED_SEGMENT_SIZE);//lux:刚好把头部去掉（因为上面已经占用了）
		ZEND_MM_MARK_FIRST_BLOCK(best_fit);//lux:标记第一块zend_mm_free_block,(修改其内部的info._prev为特殊标记)

		block_size = segment_size - ZEND_MM_ALIGNED_SEGMENT_SIZE - ZEND_MM_ALIGNED_HEADER_SIZE;

		ZEND_MM_LAST_BLOCK(ZEND_MM_BLOCK_AT(best_fit, block_size));//best_fit指向上面标记的第一个free_block（越过segment)
        //ZEND_MM_BLOCK_AT宏，定位到的是最后一个zend_free_block的位置。|segment|firstFreeBlock|***|lastFreeBlock|. ZEND_MM_LAST_BLOCK标记最后一块可用内存
	} else {//lux:找到了最符合的一个内存块，标记为使用，并从可用列表移除
zend_mm_finished_searching_for_block:
		/* remove from free list */
		ZEND_MM_CHECK_MAGIC(best_fit, MEM_BLOCK_FREED);//lux:检查是否是可用的，不可用就出大事了
	...
	remaining_size = block_size - true_size;//lux:找到的内存块是能找到的最合适的，但不一定是一样大小的（很多情况下）。比如要的是69byte，拿到的是26998byte...

	if (remaining_size < ZEND_MM_ALIGNED_MIN_HEADER_SIZE) {//剩余部分太小了，整块返回吧
		...
		ZEND_MM_BLOCK(new_free_block, ZEND_MM_FREE_BLOCK, remaining_size);//lux:标记剩余内存块

		/* add the new free block to the free list */
		if (EXPECTED(!keep_rest)) {
			zend_mm_add_to_free_list(heap, new_free_block);//lux:加入到free
		} else {
			zend_mm_add_to_rest_list(heap, new_free_block);//lux:加入到rest
		}
	}
...
{//lux:zend提供的上层内存分配函数，内存分配入口
	...
	//lux:start
	FILE *stream;
	if((stream = fopen("/tmp/php5617_emalloc.log", "a+")) != NULL){
		fprintf(stream, "1\n");//lux:简单的一个function.php 调用了1726次
	}
	fclose(stream);
	//lux:end
