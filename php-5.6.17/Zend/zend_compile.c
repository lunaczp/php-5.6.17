...
static void build_runtime_defined_function_key(zval *result, const char *name, int name_length TSRMLS_DC) /* {{{ */
{
	...
	Z_STRLEN_P(result) = 1+name_length+strlen(filename)+char_pos_len;
 	...
 	sprintf(Z_STRVAL_P(result)+1, "%s%s%s", name, filename, char_pos_buf);//lux：保存了函数名、文件名、源代码中函数定义开始的内存地址。如function a()...,则是a的地址
...
{//lux: begin function 当解析到function a...
...
{//lux: end function declaration
	...
	CG(active_op_array) = function_token->u.op_array;//lux:函数定义结束，该把active_op_array 重置回来了
...
{//lux:函数调用，如a()
	...
	if ((zend_hash_find(CG(function_table), lcname, Z_STRLEN(function_name->u.constant)+1, (void **) &function)==FAILURE) ||
 		...
 		(function->type == ZEND_INTERNAL_FUNCTION))) {//lux: 通过名字查找函数
	...
	zend_push_function_call_entry(function TSRMLS_CC);//lux: 加入函数调用堆栈
...
{//lux: 函数调用结束。如a(),遇到)
	...
	zend_stack_del_top(&CG(function_call_stack));//lux: 调用已经结束 从栈中删除
}
/* }}} */
...
{//lux: 添加用于return的zend_op
	...
	opline->opcode = returns_reference ? ZEND_RETURN_BY_REF : ZEND_RETURN;//lux: 添加用于return的zend_op
...
//lux: 比如function a(){echo 1;} 到这一步的时候，op1 对应的是function a的实际定义（op array）；op2对应的是a（变量名），下面就把a指向其实现
	zend_hash_quick_find(function_table, Z_STRVAL_P(op1), Z_STRLEN_P(op1), Z_HASH_P(op1), (void *) &function);//lux: 先根据op1找到函数入口
	if (zend_hash_quick_add(function_table, Z_STRVAL_P(op2), Z_STRLEN_P(op2)+1, Z_HASH_P(op2), function, sizeof(zend_function), NULL)==FAILURE) {//lux: 把a加入函数表，并把其值设置为函数实现的指针（入口地址）,实际操作是拷贝了一个zend_function过去
...
{//lux: 早期绑定／编译器绑定。把函数／类实现和具体变量绑定(帮变量指向对应的实现)
	zend_op *opline = &CG(active_op_array)->opcodes[CG(active_op_array)->last-1];
	HashTable *table;
	...
	switch (opline->opcode) {
        case ZEND_DECLARE_FUNCTION://lux: 自定义函数
	...
	zend_hash_quick_del(table, Z_STRVAL(CONSTANT(opline->op1.constant)), Z_STRLEN(CONSTANT(opline->op1.constant)), Z_HASH_P(&CONSTANT(opline->op1.constant)));//lux: 把函数实现删除掉（已经拷贝了一份并关联到对应变量）
	zend_del_literal(CG(active_op_array), opline->op1.constant);//lux：把保存的中间变量清除
	zend_del_literal(CG(active_op_array), opline->op2.constant);//lux: 如上
	MAKE_NOP(opline);//lux:重置opline，因为函数定义在该阶段（编译／此法语法解析阶段）已经处理完。到最终执行的时候，这条语句就不应该执行了（因为是定义，不是调用）。
}
/* }}} */
...
{/*lux: 注册全局变量 _GET _POST _COOKIE _SERVER _FILE ... */
	if (auto_global->jit) {
		auto_global->armed = 1;/* lux: just in time, 可以直接返回 */
...
again:
	...
	retval = lex_scan(&zendlval->u.constant TSRMLS_CC);//lux:lex真正开始
	switch (retval) {
		case T_COMMENT:
		case T_DOC_COMMENT:
		case T_OPEN_TAG:
