...
{//lux: zend 函数执行(如类的方法,etc)
	...
	*fci->retval_ptr_ptr = NULL;
	...
	if (EG(current_execute_data)) {//lux: 先备份
	...
	if (!fci_cache || !fci_cache->initialized) {//lux: 不合法？
		char *callable_name;
		char *error = NULL;
	...
	if (EX(function_state).function->common.fn_flags & (ZEND_ACC_ABSTRACT|ZEND_ACC_DEPRECATED)) {//lux: 不合法？
	...
	for (i=0; i<fci->param_count; i++) {
		...
		if (ARG_SHOULD_BE_SENT_BY_REF(EX(function_state).function, i + 1)) {//lux: 参数入栈
	...
	zend_vm_stack_push((void*)(zend_uintptr_t)fci->param_count TSRMLS_CC);//lux: 参数数量入栈

	current_scope = EG(scope);//lux: 备份
	...
	if (fci->object_ptr) {
		if ((EX(function_state).function->common.fn_flags & ZEND_ACC_STATIC)) {
			...
			EG(This) = fci->object_ptr;//lux: 当前作用域
	...
	if (EX(function_state).function->type == ZEND_USER_FUNCTION) {
		...
		if (EG(active_op_array)->fn_flags & ZEND_ACC_GENERATOR) {
			*fci->retval_ptr_ptr = zend_generator_create_zval(EG(active_op_array) TSRMLS_CC);
			...
			zend_execute(EG(active_op_array) TSRMLS_CC);//lux: 终于执行了
		}

		if (!fci->symbol_table && EG(active_symbol_table)) {
			zend_clean_and_cache_symbol_table(EG(active_symbol_table) TSRMLS_CC);
		}
		EG(active_symbol_table) = calling_symbol_table;//lux: 还原
	...
	zend_vm_stack_clear_multiple(0 TSRMLS_CC);//lux: 参数栈清理

	if (EG(This)) {
		zval_ptr_dtor(&EG(This));
	}
	EG(called_scope) = current_called_scope;//lux: 恢复
