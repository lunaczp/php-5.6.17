...
#define GC_BLACK  0x00//lux:正常
#define GC_WHITE  0x01//lux:垃圾
#define GC_GREY   0x02//lux:已经refcount-1
#define GC_PURPLE 0x03//lux:已经发入buf
//lux:这种把颜色属性写到gc_root_buffer的内存处的方法，节省了空间，但前提是gc_root_buffer的地址不会用到末2个bit(都为0) todo 如何实现的，应该是分配内存初始化的时候
#define GC_ADDRESS(v) \
	((gc_root_buffer*)(((zend_uintptr_t)(v)) & ~GC_COLOR))//lux:这就能看到，最后末2个bit一定是0。但是是如何保证内存地址本身最后两位一定是0呢。
...
typedef struct _gc_root_buffer {
	...
	zend_object_handle        handle;	/* must be 0 for zval               */ //如果是对象，则代表其在全局表中的index: EG(objects_store).object_buckets[handle]
	union {
		zval                 *pz;//lux:对应的zval
