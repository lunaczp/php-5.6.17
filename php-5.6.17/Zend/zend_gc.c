...
{//lux: zval 尝试加入 root 列表
	...
	if (GC_ZVAL_GET_COLOR(zv) != GC_PURPLE) {//lux: 不是紫色，则加入
		if (!GC_ZVAL_ADDRESS(zv)) {
			...
			GC_ZVAL_SET_PURPLE(zv);//lux:标记紫色，并把新节点插入roots链表表头
...
{//lux: object 尝试加入 root 列表
	...
	if (GC_GET_COLOR(obj->buffered) != GC_PURPLE) {//lux: 不是紫色，则加入
		if (!GC_ADDRESS(obj->buffered)) {
			...
			GC_SET_PURPLE(obj->buffered);//lux: 标记紫色，并把新节点插入roots链表表头
			newRoot->next = GC_G(roots).next;
			newRoot->prev = &GC_G(roots);
			GC_G(roots).next->prev = newRoot;
			GC_G(roots).next = newRoot;
...
{//lux: object 紫 转移为 灰
	...
	if (GC_GET_COLOR(obj->buffered) != GC_GREY) {
		...
		if (EXPECTED(EG(objects_store).object_buckets[Z_OBJ_HANDLE_P(pz)].valid &&
			...
			int i, n;
			zval **table;
			HashTable *props = get_gc(pz, &table, &n TSRMLS_CC);//lux: 获取对象属性
			...
			p = props->pListHead;
			while (p != NULL) {//lux: 对每一个属性，尝试mark gray
				pz = *(zval**)p->pData;
				if (Z_TYPE_P(pz) != IS_ARRAY || Z_ARRVAL_P(pz) != &EG(symbol_table)) {
					pz->refcount__gc--;
				}
...
{//lux: 遍历root列表，紫 转移为 灰，其他的移出root 列表
	gc_root_buffer *current = GC_G(roots).next;

	while (current != &GC_G(roots)) {//lux: 双链表遍历
		if (current->handle) {//lux: 对象
			if (EG(objects_store).object_buckets) {
				...
				if (GC_GET_COLOR(obj->buffered) == GC_PURPLE) {
					...
					zobj_mark_grey(obj, &z TSRMLS_CC);//lux: 标记为灰
				} else {
			...
			if (Z_TYPE_P(pz) == IS_OBJECT && EG(objects_store).object_buckets) {
				...
				if (GC_GET_COLOR(obj->buffered) == GC_GREY) {
					if (obj->refcount > 0) {
						...
						GC_SET_COLOR(obj->buffered, GC_WHITE);//lux: 标记为白色
						if (EXPECTED(EG(objects_store).object_buckets[Z_OBJ_HANDLE_P(pz)].valid &&
						             (get_gc = Z_OBJ_HANDLER_P(pz, get_gc)) != NULL)) {
							int i, n;
							zval **table;
...
{//lux: object 灰 转移为 白或黑
	...
	if (EG(objects_store).object_buckets) {
		...
		if (GC_GET_COLOR(obj->buffered) == GC_GREY) {
			if (obj->refcount > 0) {
				zobj_scan_black(obj, pz TSRMLS_CC);//lux: 扫描black
			} else {
				GC_SET_COLOR(obj->buffered, GC_WHITE);//lux: 标记为白色
				if (EXPECTED(EG(objects_store).object_buckets[Z_OBJ_HANDLE_P(pz)].valid &&
				             (get_gc = Z_OBJ_HANDLER_P(pz, get_gc)) != NULL)) {//lux:对object对属性进行scan
					int i, n;
					zval **table;
					HashTable *props = get_gc(pz, &table, &n TSRMLS_CC);

					for (i = 0; i < n; i++) {//lux: 扫描获取的一堆zval
						if (table[i]) {
							pz = table[i];
							zval_scan(pz TSRMLS_CC);
                    	}
					...
					p = props->pListHead;
					while (p != NULL) {//lux: 扫描获取的属性里的zval
						zval_scan(*(zval**)p->pData TSRMLS_CC);
						p = p->pListNext;
					}
				}
...
tail_call:
	if (((zval_gc_info*)(pz))->u.buffered == (gc_root_buffer*)GC_WHITE) {
			...
			if (obj->buffered == (gc_root_buffer*)GC_WHITE) {
				...
				if (EXPECTED(EG(objects_store).object_buckets[Z_OBJ_HANDLE_P(pz)].valid &&
					...
					if (!props) {
						...
						pz->refcount__gc++;
						((zval_gc_info*)pz)->u.next = GC_G(zval_to_free);//lux: 挂到free列表（只改变指针，因而free列表不占大量内存，只记录表头即可）
		...
		/* restore refcount and put into list to free */
		pz->refcount__gc++;
		((zval_gc_info*)pz)->u.next = GC_G(zval_to_free);//lux:加入free列表
		GC_G(zval_to_free) = (zval_gc_info*)pz;
	...
	while (current != &GC_G(roots)) {
		if (current->handle) {//lux: 对象回收
			if (EG(objects_store).object_buckets) {
				...
				Z_OBJ_HANDLE(z) = current->handle;
				Z_OBJ_HT(z) = current->u.handlers;
				zobj_collect_white(&z TSRMLS_CC);//lux: 回收白色
			}
		} else {//lux: zval回收
			GC_ZVAL_SET_ADDRESS(current->u.pz, NULL);
			zval_collect_white(current->u.pz TSRMLS_CC);//lux: 回收白色
		}

		GC_REMOVE_FROM_BUFFER(current);//lux: 从 root 列表移除；循环结束后，root 列表就空了；需要被free的节点已经加入到了zval_to_free 列表
		current = current->next;
	}
}
...
{//lux: 垃圾回收入口
	...
	if (GC_G(roots).next != &GC_G(roots)) {
		...
		GC_G(gc_active) = 1;
		gc_mark_roots(TSRMLS_C);//lux: 紫色转移为灰色
		gc_scan_roots(TSRMLS_C);//lux: 灰色转移为白色|黑色
		gc_collect_roots(TSRMLS_C);//lux: 回收白色
		...
		GC_G(zval_to_free) = NULL;
		GC_G(gc_active) = 0;
        //lux: 上面收集好了要销毁的(在zval_to_free)，下面开始销毁。
		/* First call destructors */ //lux: 调用对象自身的析构函数（zval指向的对象）
		...
		/* Destroy zvals */ //lux: 销毁对象本身（zval指向的对象）
		p = GC_G(free_list);
		while (p != FREE_LIST_END) {
			...
			if (Z_TYPE(p->z) == IS_OBJECT) {
				if (EG(objects_store).object_buckets &&
					...
					Z_TYPE(p->z) = IS_NULL;
					zend_objects_store_del_ref_by_handle_ex(Z_OBJ_HANDLE(p->z), Z_OBJ_HT(p->z) TSRMLS_CC);//lux: 销毁该zval对应的object store中的对象
				}
