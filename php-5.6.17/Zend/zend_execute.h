...
{//lux:变量析构
	if (!Z_DELREF_P(zval_ptr)) {//lux:refcount=0 直接清理
		ZEND_ASSERT(zval_ptr != &EG(uninitialized_zval));
		GC_REMOVE_ZVAL_FROM_BUFFER(zval_ptr);//lux: 从gc buffer移除
		zval_dtor(zval_ptr);//lux: zval的析构(根据不同变量类型，去析构)
		efree_rel(zval_ptr);//lux: zval变量本身的内存释放
	} else {
		if (Z_REFCOUNT_P(zval_ptr) == 1) {//lux:重置is_ref
			Z_UNSET_ISREF_P(zval_ptr);
		}

		GC_ZVAL_CHECK_POSSIBLE_ROOT(zval_ptr);//lux:尝试放入gc
	}
}
