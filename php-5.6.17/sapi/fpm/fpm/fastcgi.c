...
{/* lux:具体逻辑见：https://github.com/lunaczp/spec/blob/master/spec.md#52-name-value-pair-streams-fcgi_params */
	size_t ret = 0;

	if (p < end) {
		*result = p[0];
		if (*result < 128) {/* lux: 一位 */
			ret = 1;
		}
		else if (p + 3 < end) { /* lux: 四位 */
...
{ /* lux: 从输入流解析出参数 */
	...
	while (p < end) {/* lux: buf是fastcgi定义的Name-Value pairs */
		bytes_consumed = fcgi_get_params_len(&name_len, p, end); /* lux:参数长度 */
		if (!bytes_consumed) {
			/* Malformated request */
			ret = 0;
			break;
		...
		p += bytes_consumed;
		bytes_consumed = fcgi_get_params_len(&val_len, p, end); /* lux:值长度 */
		if (!bytes_consumed) {
			/* Malformated request */
			ret = 0;
			break;
		 ...
		 */
		if (!fcgi_param_get_eff_len(p, p+name_len, &eff_name_len)){ /* lux:参数名中间不能包含结束符\0 */
			/* Malicious request */
			ret = 0;
			break;
		}
		...
		memcpy(tmp, p, eff_name_len); /* lux:名，如"SCRIPT_FILENAME" */
		tmp[eff_name_len] = 0;
		s = estrndup((char*)p + name_len, val_len); /* lux:值 */
		if (s == NULL) {
			ret = 0;
			break;
		}
...
{//lux: 读请求 解析fastcgi parameter
	...
	if (safe_read(req, &hdr, sizeof(fcgi_header)) != sizeof(fcgi_header) || /* lux：读fastcgi record 头 */
	    hdr.version < FCGI_VERSION_1) {
		return 0;
	}
	...
	if (hdr.type == FCGI_BEGIN_REQUEST && len == sizeof(fcgi_begin_request)) {/* lux: 这是一个FCGI_BEGIN_REQUEST */
		char *val;
		...
		while (hdr.type == FCGI_PARAMS && len > 0) {/* lux: 这是一个FCGI_PARAMS record */
			if (len + padding > FCGI_MAX_LENGTH) {
				return 0;
			}
			...
			if (!fcgi_get_params(req, buf, buf+len)) {/* lux:参数解析 */
				req->keep = 0;
				return 0;
			}
	...
	} else if (hdr.type == FCGI_GET_VALUES) { /* lux: 这个是一个FCGI_GET_VALUES 请求 */
...
{//lux: 从socket 接收请求，并解析参数，记录到req
	...
	while (1) { /* {{{ */
		...
		if (fcgi_read_request(req)) { /* lux:  解析并填充request */
#ifdef _WIN32
			if (is_impersonate && !req->tcp) {
...
{//lux: finish and flush
	int ret = 1;

	if (req->fd >= 0) {
		if (!req->closed) {
			ret = fcgi_flush(req, 1);/* lux: flush request */
			req->closed = 1;
		}
