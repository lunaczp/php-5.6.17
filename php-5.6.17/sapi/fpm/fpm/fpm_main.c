...
{/* lux: 用fastcgi解析的req信息来初始化request_info */
...
<a href=\"http://php.net/install.windows\">the manual page</a>.</p>\n");
	...
	parent = 0;//lux: 父进程不会从fpm_run返回，直接进入event loop。子进程会返回，往下继续处理request 事件
	...
	zend_first_try {
		while (fcgi_accept_request(&request) >= 0) {//lux：开始处理请求
			...
			request_body_fd = -1;
			SG(server_context) = (void *) &request;/* lux: 存到全局 */
			init_request_info(TSRMLS_C);
			CG(interactive) = 0;

			fpm_request_info(); /* lux: scoreboard 初始化 */

			/* request startup only after we've done all we can to
			 *            get path_translated */
			if (php_request_startup(TSRMLS_C) == FAILURE) {/* lux: 统一的初始化 */
			...
			if (fpm_status_handle_request(TSRMLS_C)) {/* 内置status输出 */
				goto fastcgi_request_done;
			}
			...
			if (fpm_php_limit_extensions(SG(request_info).path_translated)) {
				SG(sapi_headers).http_response_code = 403;/* lux:后缀限制，不合法则返回403 */
				PUTS("Access denied.\n");
				goto fastcgi_request_done;
			}
			...
			if (php_fopen_primary_script(&file_handle TSRMLS_CC) == FAILURE) {
				zend_try {
					...
					if (errno == EACCES) {
						SG(sapi_headers).http_response_code = 403;/* lux: 打开文件权限问题 */
						PUTS("Access denied.\n");
					} else {
						SG(sapi_headers).http_response_code = 404;
			...
			php_execute_script(&file_handle TSRMLS_CC);/* lux: 执行 */
...
fastcgi_request_done:
			...
			php_request_shutdown((void *) 0);//lux: done and done

			requests++;
			if (max_requests && (requests == max_requests)) {//lux: 已经处理了很多请求了，达到约束，退出
				fcgi_finish_request(&request, 1);
				break;
			}
			/* end of fastcgi loop */
