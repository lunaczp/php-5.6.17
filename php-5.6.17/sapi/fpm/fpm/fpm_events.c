...
{//lux: event loop
	...
	while (1) {
		...
		/* search in the timeout queue for the next timer to trigger */ /*lux：取最小的（最快out的） */
		...
		ret = module->wait(fpm_event_queue_fd, timeout);/* lux: 等待并处理由系统IO触发的事件 */
		...
		while (q) {/* lux: 遍历处理那些timeout的事件 */
			...
			if (q->ev) {
				if (timercmp(&now, &q->ev->timeout, >) || timercmp(&now, &q->ev->timeout, ==)) {
					...
					if (q->ev->flags & FPM_EV_PERSIST) {//因为是persist，重置timeout，然后等待下一次唤醒
						fpm_event_set_timeout(q->ev, now);
					} else { /* delete the event */ //lux:否则就删除
						q2 = q;
						if (q->prev) {
							q->prev->next = q->next;
						}
