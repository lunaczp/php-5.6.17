...
#ifdef PHP_WRITE_STDOUT
	do {
		ret = write(STDOUT_FILENO, str, str_length);//lux: 调用c write函数，写入啦
...
{
	if (php_module_startup(sapi_module, NULL, 0)==FAILURE) {//lux:调用通用的方法来启动php cli module。这里的sapi_module是封装的cli module信息
		return FAILURE;
	}
	return SUCCESS;
}
		...
		*arg_excp = arg_free; /* reconstuct argv */
		...
		case PHP_MODE_STANDARD:
			...
			if (interactive && cli_shell_callbacks.cli_shell_run) {
				...
				php_execute_script(&file_handle TSRMLS_CC);/*lux: exe */
...
exit_loop:
	...
	sapi_startup(sapi_module);//lux：初始化本模块
	sapi_started = 1;
	...
	if (sapi_module->startup(sapi_module) == FAILURE) {//lux: 启动该模块。 php_cli module(sapi module的一种,其他如fpm,cgi...) startup
	...
	zend_first_try {
		...
		if (sapi_module == &cli_sapi_module) {
#endif
			exit_status = do_cli(argc, argv TSRMLS_CC);//lux: sapi模块已经启动，下面就是各自的业务代码，这里是cli
#ifndef PHP_CLI_WIN32_NO_CONSOLE
		} else {
