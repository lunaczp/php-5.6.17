...
{/* lux:event事件处理，回调 */
	...
	for (fd=0 ; fd<=max_fd ; fd++)  {
		...
		if (PHP_SAFE_FD_ISSET(fd, &poller->active.wfds)) {
                if (SUCCESS != callback(opaque, fd, POLLOUT)) {/* lux:输出事件(http response)  */
                    retval = FAILURE;
                }
		}
	}
...
{/* lux: 静态文件 */
	...
	php_cli_server_poller_add(&server->poller, POLLOUT, client->sock);/*lux:添加输出事件 */
	return SUCCESS;
}
/* }}} */
...
static int php_cli_server_ctor(php_cli_server *server, const char *addr, const char *document_root, const char *router TSRMLS_DC) /* {{{ */
{
	...
	php_cli_server_poller_add(&server->poller, POLLIN, server_sock);/*lux：添加socket监听事件 */
...
{/* lux: 事件回调 */
	...
	if (server->server_sock == fd) {
		...
		php_cli_server_recv_event_read_request(server, client TSRMLS_CC);/*lux: 处理请求 */
	} else {
		php_cli_server_client **client;
		if (SUCCESS == zend_hash_index_find(&server->clients, fd, (void **)&client)) {
			...
			if (event & POLLOUT) {
				params->whandler(server, *client TSRMLS_CC); /*lux:  输出 */
			}
		}
	}
	return SUCCESS;
