...
{/* lux: 底层通用初始化方法 */
	...
	if (arg == PARSE_POST) {
		sapi_handle_post(array_ptr TSRMLS_CC); /*lux: handle post*/
		return;
	}
...
PHPAPI int php_hash_environment(TSRMLS_D)
{
	...
	zend_activate_auto_globals(TSRMLS_C);/*lux:注册全局变量 */
...
static zend_bool php_auto_globals_create_request(const char *name, uint name_len TSRMLS_DC)
{
	...
	for (; p && *p; p++) {/*lux:顺序按照variables_order的配置，相同的变量，后面的会覆盖前面的。此版本是EGPCS */
		switch (*p) {
			case 'g':
			case 'G':
				if (!_gpc_flags[0]) {
