...
{//lux: 所有的sapi请求启动的入口(初始化一些全局变量如_GET _POST，以及其他准备工作)。请求到来时，先调用这里初始化，然后去处理具体文件
	int retval = SUCCESS;
	...
	zend_try {
		...
		php_hash_environment(TSRMLS_C);/* lux:初始化各种变量 */
		zend_activate_modules(TSRMLS_C);
		PG(modules_activated)=1;
	} zend_catch {
		retval = FAILURE;
...
{//lux: 注册php扩展(pcre...)，内置函数(见internal_functions.c:38)等模块
...
{//lux: 所有sapi模块的启动入口
	...
	module_shutdown = 0;
	module_startup = 1;
	sapi_initialize_empty_request(TSRMLS_C);/*lux: module startup阶段，并没有request */
	sapi_activate(TSRMLS_C);/*lux: 这里并没有请求，到request startup阶段，会再次调用sapi_active */
	...
	zend_startup(&zuf, NULL TSRMLS_CC);/* lux: zend 启动 */
	...
	/*lux 注册stream wrapper for tcp,udp,...,所有的socket通信依赖于此
	...
	php_startup_auto_globals(TSRMLS_C);//lux:注册PHP全局变量
	zend_set_utility_values(&zuv);
	php_startup_sapi_content_types(TSRMLS_C);//lux:注册sapi文档类型

	/* startup extensions statically compiled in */
	if (php_register_internal_extensions_func(TSRMLS_C) == FAILURE) {//lux:注册默认扩展，包括内置函数，date扩展... 见internal_functions_cli.c:38 internal_functions.c:38,,,
	 ...
	 */
	php_ini_register_extensions(TSRMLS_C);//lux: 加载配置文件中的拓展
	zend_startup_modules(TSRMLS_C);//lux: 让zend来真正加载、初始化我们刚刚注册的模块

	/* start Zend extensions */
	zend_startup_extensions();//lux: zend extension
	...
	module_initialized = 1;//lux: 至此，模块初始化完毕
