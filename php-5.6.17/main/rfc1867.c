...
static int multipart_buffer_headers(multipart_buffer *self, zend_llist *header TSRMLS_DC)
{
	...
	while( (line = get_line(self TSRMLS_CC)) && line[0] != '\0' )
	{
		...
		if (value) { /* lux: 如 Content-Disposition */
			if(buf_value.c && key) {
		...
		if ((cd = php_mime_get_hdr_value(header, "Content-Disposition"))) {
			...
			while (*cd && (pair = getword(mbuff->input_encoding, &cd, ';' TSRMLS_CC)))
			{
				...
				if (strchr(pair, '=')) {
					...
					} else if (!strcasecmp(key, "filename")) {/*lux: 找到filename */
						if (filename) {
							efree(filename);
						}
			...
			if (!cancel_upload) {
				...
				{
#endif
					fd = php_open_temporary_fd_ex(PG(upload_tmp_dir), "php", &temp_filename, 1 TSRMLS_CC);/*lux: 创建临时文件 */
					upload_cnt--;
					if (fd == -1) {
			...
			while (!cancel_upload && (blen > 0))
			{
				...
				if (PG(upload_max_filesize) > 0 && (long)(total_bytes+blen) > PG(upload_max_filesize)) {
					...
					wlen = write(fd, buff, blen);/* lux: 写入临时文件 */
