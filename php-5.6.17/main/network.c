...
PHPAPI int php_network_connect_socket(php_socket_t sockfd,
		...
		char **error_string,
		int *error_code)
{/*lux socket:connect 区分系统是否支持非阻塞模式（异步），如果不支持则直接connect（此时，timetout是没用的）。支持，则具体处理见下面。注意此时如果connect失败，需要对失败的情况做具体分析才能知道是否真正成功或者失败，参考：http://man7.org/linux/man-pages/man2/connect.2.html */
...
#else
	if ((n = php_pollfd_for(sockfd, PHP_POLLREADABLE|POLLOUT, timeout)) == 0) {/*lux:需要进一步判断是成功还失败 http://man7.org/linux/man-pages/man2/connect.2.html */
#endif
...
php_socket_t php_network_connect_socket_to_host(const char *host, unsigned short port,
		...
		TSRMLS_DC)
{/*lux socket:connect 连接到远程主机。先调用connect，如果指定了本地的bind端口，也调用bind*/
