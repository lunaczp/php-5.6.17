...
/*lux stream:create 建立连接（先构造并初始化stream对象，然后调用相应方法建立连接）。对于socket类型而言，会完成建立socket，connect（for client），bind（for server），listen（for server）操作。*/	
		...
		STREAMS_DC TSRMLS_DC)
{
	...
	if (timeout == NULL) {
		timeout = &default_timeout;/*如果没设置timeout，则取默认配置default_socket_timeou*/
	}
	...
	/*lux 调用工厂方法，创建stream对象，比如protocol是tcp，则调用的是php_stream_generic_socket_factory
	* 这是在php_module_startup => php_init_stream_wrappers注册的，参见main/streams/streams.c:1650
	* 此时只是创建对象，注册函数等等，并没有连接，下面开始连接
	*/
	...
	if (stream) {
		...
		if ((flags & STREAM_XPORT_SERVER) == 0) {
			...
			if (flags & (STREAM_XPORT_CONNECT|STREAM_XPORT_CONNECT_ASYNC)) {/*lux client 建立连接（first create socket,then connect）*/
			...
			if (flags & STREAM_XPORT_BIND) {
				if (0 != php_stream_xport_bind(stream, name, namelen, &error_text TSRMLS_CC)) {/*lux server bind(first creat socket,then bind)*/
					...
					if (0 != php_stream_xport_listen(stream, backlog, &error_text TSRMLS_CC)) {/*lux server listen*/
	...
	return stream;/*lux 初始化完毕。对于client而言已经connect；对于server而言已经listen*/
}
...
PHPAPI int php_stream_xport_connect(php_stream *stream,
		...
		int *error_code
		TSRMLS_DC)
{/*lux stream:connect 通用连接入口函数*/
	php_stream_xport_param param;
	int ret;
	
	memset(&param, 0, sizeof(param));
	param.op = asynchronous ? STREAM_XPORT_OP_CONNECT_ASYNC: STREAM_XPORT_OP_CONNECT;/*lux 连接指令，异步还是同步*/
	param.inputs.name = (char*)name;
	param.inputs.namelen = namelen;
	...
	ret = php_stream_set_option(stream, PHP_STREAM_OPTION_XPORT_API, 0, &param);/*lux 执行连接指令，设置参数，最终会调用connect(,bind)*/
