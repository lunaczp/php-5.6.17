...
PHPAPI int php_stream_from_persistent_id(const char *persistent_id, php_stream **stream TSRMLS_DC)
{
	...
	/*lux 先在persistent_list找该persisten_id,没有则返回不存在。有但类型不对，则返回失败。否则返回成功（返回之前在regular_list中找，找到则增加计数；没找到则插入）*/
	if (zend_hash_find(&EG(persistent_list), (char*)persistent_id, strlen(persistent_id)+1, (void*) &le) == SUCCESS) {
		if (Z_TYPE_P(le) == le_pstream) {/*lux 判断类型*/
			if (stream) {
				HashPosition pos;
...
/* }}} */
/*lux 初始化一个stream类，以后的操作就可以基于stream来做。ops标示了是tcp,udp或其他类型，并提供了对应的读写函数；abstract空指针，可以指向具体的对象比如php_netstream_data_t对象（socket） */
...
fprintf(stderr, "stream_alloc: %s:%p persistent=%s\n", ops->label, ret, persistent_id);
	...
	if (persistent_id) {
		...
		Z_TYPE(le) = le_pstream;/*lux 设置该条目的类型，以后会用来做判断。注意le_pstaem的值在php_init_stream_wrappers初始化的时候设置*/
		le.ptr = ret;
		le.refcount = 0;
		/*lux 注册persistent_id到全局变量persistent_list中，方便以后复用*/
	...
	ret->rsrc_id = ZEND_REGISTER_RESOURCE(NULL, ret, persistent_id ? le_pstream : le_stream);/*注册stream对象到全局regular_list列表*/
...
fprintf(stderr, "stream_free: %s:%p[%s] preserve_handle=%d release_cast=%d remove_rsrc=%d\n",
	...
	_php_stream_flush(stream, 1 TSRMLS_CC);/*lux: fclose,file_get_contents will arrive here*/
...
{/*lux: flush: fclose file_put_contents will arrive here*/
	int ret = 0;
...
/*lux stream set option 通用入口，其实会调用各自注册好的方法*/
PHPAPI int _php_stream_set_option(php_stream *stream, int option, int value, void *ptrparam TSRMLS_DC)
{
	int ret = PHP_STREAM_OPTION_RETURN_NOTIMPL;

	if (stream->ops->set_option) {
		ret = stream->ops->set_option(stream, option, value, ptrparam TSRMLS_CC);/*lux 调用各自的方法，一般是php_sockop_set_option，参见main/streams/xp_socket.c:447*/
	}
...
/*lux 初始化stream wrapper，注册tcp,udp,...*/
int php_init_stream_wrappers(int module_number TSRMLS_DC)
{
	...
	le_pstream = zend_register_list_destructors_ex(NULL, stream_resource_persistent_dtor, "persistent stream", module_number);/*lux le_pstream的值被设置为list_destructors列表内persisten stream所在的偏移*/
