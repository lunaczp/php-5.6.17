...
/*lux socket:通用 set option 入口。注意tcp不走这里，而是走php_tcp_sockop_set_option，参见本页447*/
static int php_sockop_set_option(php_stream *stream, int option, int value, void *ptrparam TSRMLS_DC)
{
	...
	switch(option) {
		...
		case PHP_STREAM_OPTION_READ_TIMEOUT:/*lux: 读操作超时*/
		...
		case PHP_STREAM_OPTION_XPORT_API:/*lux 来自xport的配置请求*/
			xparam = (php_stream_xport_param *)ptrparam;

			switch (xparam->op) {
				case STREAM_XPORT_OP_LISTEN:
					xparam->outputs.returncode = (listen(sock->socket, xparam->inputs.backlog) == 0) ?  0: -1;/*lux: socket listen*/
					return PHP_STREAM_OPTION_RETURN_OK;
		...
		php_stream_xport_param *xparam TSRMLS_DC)
{
...
#ifdef AF_UNIX /*lux unix domin socket*/
	if (stream->ops == &php_stream_unix_socket_ops || stream->ops == &php_stream_unixdg_socket_ops) {
		struct sockaddr_un unix_addr;
	...
	sock->socket = php_network_connect_socket_to_host(host, portno,
			...
			bindto,
			bindport
			TSRMLS_CC);/*lux 连接，并把返回的句柄保存在socket（其实是phpstream->abstract->socket)*/
...
/*lux socket:tcp:set option函数，用来建立连接或者设定选项。connect? bind? accept? 或者是普通的option指令*/
static int php_tcp_sockop_set_option(php_stream *stream, int option, int value, void *ptrparam TSRMLS_DC)
{
	php_netstream_data_t *sock = (php_netstream_data_t*)stream->abstract;
	php_stream_xport_param *xparam;
		...
		case PHP_STREAM_OPTION_XPORT_API:
			...
			switch(xparam->op) {
				...
				case STREAM_XPORT_OP_CONNECT_ASYNC:
					xparam->outputs.returncode = php_tcp_sockop_connect(stream, sock, xparam TSRMLS_CC);/*lux socket:tcp:connect*/
					return PHP_STREAM_OPTION_RETURN_OK;

				case STREAM_XPORT_OP_BIND:
					xparam->outputs.returncode = php_tcp_sockop_bind(stream, sock, xparam TSRMLS_CC);/*lux socket:tcp:bind*/
					return PHP_STREAM_OPTION_RETURN_OK;


				case STREAM_XPORT_OP_ACCEPT:
					xparam->outputs.returncode = php_tcp_sockop_accept(stream, sock, xparam STREAMS_CC TSRMLS_CC);/*lux socket:tcp:accept 接收到的client会放置在xparam->outputs.client*/
					return PHP_STREAM_OPTION_RETURN_OK;
				default:/*lux 这里没有单独对listen的解析，在下面的函数里*/
					/* fall through */
					;
			}
	}
	return php_sockop_set_option(stream, option, value, ptrparam TSRMLS_CC);/*lux 以上是tcp相关的参数，这里设置其他通用指令（listen）和普通参数*/
}

/*lux stream:socket:factory 通用socket工厂实现，用于tcp,udp等，返回一个新的php_stream对象（对象有一个php_netstream_data_t成员，用于存储socket信息）。*/
PHPAPI php_stream *php_stream_generic_socket_factory(const char *proto, size_t protolen,
		const char *resourcename, size_t resourcenamelen,
		const char *persistent_id, int options, int flags,
		struct timeval *timeout,
		php_stream_context *context STREAMS_DC TSRMLS_DC)
{/*lux 这里传入的timeout没有使用到，默认使用了PHP的配置default_socket_timeout，见下面*/
	...
	if (strncmp(proto, "tcp", protolen) == 0) {
		ops = &php_stream_socket_ops;/*lux  设定tcp的socket读写函数 */
	...
	sock->is_blocked = 1;
	sock->timeout.tv_sec = FG(default_socket_timeout);/*lux 使用默认配置*/
	sock->timeout.tv_usec = 0;

	/* we don't know the socket until we have determined if we are binding or
	 * connecting */
	sock->socket = -1;/*此时只是初始化内存对象，还没有connect；connect之后，这里保存的就是返回的资源id（句柄）*/
